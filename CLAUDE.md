...
│   └── flat/                        ← Generators for every in-story page illustration
│       ├── build_pages.py           ← Regenerates pages + injects into stories/*/index.html
│       ├── characters.py            ← The flat character library (main cast + walk-on patients)
│       ├── scene.py                 ← Backgrounds, bubbles, props, vehicles
│       └── pages.py                 ← One pNN_M() per story page, registered in PAGES
└── stories/
...
**Two illustration styles, on purpose** — a painted cover and simpler interior pages is ordinary picture-book convention, not an unfinished migration:

- **Generated-texture style** — `characters.html` portraits (`tools/portraits/`) and the six story covers (`tools/scenes/`). See "Character portraits" and "Story covers" below.
- **Flat style** — flat fills + thin outlines, used by every in-story page illustration (66 pages across all six stories), generated by `tools/flat/`. See "In-story pages" below. Don't try to move these pages to the generated-texture style: that was tried and dropped. The pages need new geometry (a bear lying in a hospital bed, riders in a car, a trunk on a shoulder) that a posable portrait can't provide, and one textured scene is 115–185 KB against 5–12 KB for a flat page.

**All art is generated. Never hand-edit an `<svg>` in the HTML — the next build overwrites it.** Edit the generator and rebuild.
...
**In-story pages (`stories/*/index.html`, each `.page-illustration` SVG)** are generated. To change one, edit its function in `tools/flat/pages.py` and rebuild:

```bash
python3 tools/flat/build_pages.py          # every story in PAGES
python3 tools/flat/build_pages.py 03       # just this one
```

The build leaves the cover alone (`build_covers.py` owns it) and refuses to run if a story's number of spread pages doesn't match its list in `PAGES`.
...
**Background template (standard for every flat page — `scene.sky()`/`ground()`/`page()` implement it; `indoors()` and `snow_sky()`/`snow_ground()` are its interior and winter variants):** Never fill a page background with a single flat `<rect>`. Every illustration should feel like the same warm, sunny meadow, using this layered recipe:
...
3. Add a flat version to `tools/flat/characters.py` for the story pages: copy the closest existing function and its pose table, take the palette from the portrait generator, and check it on a contact sheet with every `EXPR` entry. A one-off patient can be a new `CRITTERS` entry instead.

## Story page structure

Each story HTML has:
- `.page.page-cover` — full-width cover (index 0)
- `.page.page-spread` × N — story pages, each with `.page-illustration` (SVG, generated by `tools/flat/`) + `.page-text` (story prose)
